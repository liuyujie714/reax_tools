...
def _default_count_pages(file_name: str, table: pd.DataFrame, targets: list[str] | None, config: dict) -> list[list[tuple[str, str]]]:
    ...
    if file_name == "species_count.csv":
        ordered = _importance_order(table)[:max_per_page]
...
def plot_species_count_long(species_count: pd.DataFrame, output: str | Path, top_n: int = 8) -> None:
    config = {"max_per_page": top_n, "min_per_page": 4, "label_subscripts": True}
    plot_count_pages(species_count, output, file_name="species_count.csv", config=config)
...
def plot_reaction_events(events: pd.DataFrame, output: str | Path, top_n: int = 15, config: dict | None = None) -> None:
    if events.empty:
        return
    config = config or {}
    top = events.sort_values("frequency", ascending=False).head(top_n).copy()
    labels = [
    ...
    values = top["frequency"].astype(int).to_numpy()
